...
var aa = a*a, bb = b*b;
var twoaa = 2 * aa, twobb = 2 * bb
var x = 0, y = b;
var d, dx = 0, dy = twoaa * y;
	...
	d = Math.round( bb * ( x + 0.5) * ( x + 0.5) + aa * ( y - 1) * ( y - 1) - aa * bb +0.5);
	while( y > 0)
	{
		y --;
		dy -= twoaa;
...
var xl, xr, i, stack = new Array, pt = new Object;
var spanNeedFill;
	pt.x = x, pt.y = y;
	stack.push( pt)
var oldColor = this.GetPixel( x, y)
	...
	while( stack.length)
	{
		...
		while( this.GetPixel( x, y) == oldColor)//向右填充
		{
			this.Pixel( x, y, newColor);
			x ++;
		}
		...
		x = pt.x - 1;
		while( this.GetPixel( x, y) == oldColor)//向左填充
		{
			this.Pixel( x, y, newColor);
			x --;
		}
		...
		while( x < xr)
		{
			...
			while( this.GetPixel( x, y) != oldColor && x < xr)
				x ++;
		}
...
var xl, xr, i, stack = new Array, pt = new Object;
var pWidth = pattern.width, pHeight = pattern.height //取出图案大小
...
var oldColor = this.GetPixel( x, y)
	...
	while( stack.length)
	{
		...
		while( this.GetPixel( x, y) == oldColor)//向右填充
		{		
			this.$.putImageData( pattern, x - x % pWidth, y - y % pHeight, x % pWidth, y % pHeight, 1, 1);
			x ++;
		}
		xr = x - 1;
