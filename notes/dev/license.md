...
- 内部使用不受影响
- **未修改版本**
  - 无论内部使用还是外部提供服务，只需遵守原始许可证条款即可，不存在额外的开源义务。
- **修改后的分发/公共服务**
  - 修改了源代码，并将该修改后的版本作为**公共网络服务**提供给外部客户/实体时，要求提供对应修改部分的源代码。
...
- vs GPL
  ...
  - 不限制使用的代码的许可证
- 文件级开源
  - 只要修改的文件必须在 MPL-2.0 下开源即可，未修改或新增的文件可使用其他许可证。
- 常见误区
  - 收费：可以对软件收费，但必须提供源代码；不得额外收费提供源代码访问权限。
  - 全部代码：仅修改或新增的 MPL-2.0 文件需要开源；不要求整个项目统一为 MPL-2.0。
