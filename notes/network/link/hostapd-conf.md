...
# 提示 hostapd 指定的网络设备在桥接中 - 不指定会自动检测
# nl80211 该参数被让 hostapd 将设备添加到桥接 - 如果桥接设备不存在则创建
bridge=br0

# 驱动类型 - hostap/wired/none/nl80211/bsd - 默认 hostap
# 所有的 Linux mac80211 驱动都使用 nl80211
# driver=none 用于将 hostapd 配置为一个独立的 RADIUS 服务器
driver=hostap

# syslog 和 stdout 日志 - stdout 只用于前台运行时
# 值为位控制 - 日志等级喂 0-4 分别为 verbose、debug、info、notify、warn
# bit 0 (1) = IEEE 802.11
# bit 1 (2) = IEEE 802.1X
# bit 2 (4) = RADIUS
# bit 3 (8) = WPA
...
# 国家码 - ISO/IEC 3166-1 - dot11CountryString
# 可能会限制通道、传输功率等
#country_code=US
...
# 室内
#country3=0x49
# 无国家实体 (country_code=XX)
#country3=0x58
# IEEE 802.11 standard Annex E table indication: 0x01 .. 0x1f
# Annex E, Table E-4 (Global operating classes)
#country3=0x04

# 启用 IEEE 802.11d - 会发放国家码和允许的通道、传输速率等
# 默认 0 - 关闭
#ieee80211d=1

# 启用 IEEE 802.11h - 启用雷达检测和 DFS 支持
# DFS 需要室外 5 GHz 通道 - 配合 ieee80211d=1 使用
# 默认 0 - 关闭
#ieee80211h=1
...
# a/g = IEEE 802.11n (HT)
# IEEE 802.11ac (VHT)、IEEE 802.11ax (HE) 6 GHz 需要设置为 a
# 使用 ACS 时可设置为 any - ACS 需要设备支持
# 默认 IEEE 802.11b
hw_mode=g

# 如果启用了 ACS 可设置为 0 自动选择
# 默认 0 - 未设置
channel=1
...
# hostapd 实现基于 IEEE Std 802.1X-2004 - EAPOL 版本 2
# 如果客户端不支持 2 可设置为 1 旧版本
# MACsec 版本为 3 - IEEE Std 802.1X-2010
#eapol_version=2

# 可选的 EAP Request-Identity 展示消息
# 第一个 \0 用于分割逗号分隔的网络信息 RFC 4284.
#eap_message=hello
#eap_message=hello\0networkid=netw,nasid=foo,portid=0,NAIRealms=example.com

# WEP 密钥更换 - 如果密钥长度未设置或这职位 0 则禁用
# 默认广播和独立单播长度为:
...
# Rekeying 的周期 - 秒 - 0 为不进行 rekey
#wep_rekey_period=300

# EAPOL-Key index workaround (set bit7) for WinXP Supplicant (needed only if only broadcast keys are used)
eapol_key_index_workaround=0

# 从新授权的时间 - 默认 3600 秒 - 设置 0 禁用
#eap_reauth_period=3600

# 当使用 driver=wired 发送 EAPOL frames 时 - 使用 PAE 组地址而不是杜立德目标地址
# 悠闲鉴权的最常用方式 - 要求端口被一个基站使用
#use_pae_group_addr=1

# EAP 从新授权协议的授权者
# EAP Re-authentication Protocol (ERP) authenticator (RFC 6696)
# 是否在 EAP-Identity/Request 之前发起发起的 EAP 鉴权携带 EAP-Initiate/Re-auth-Start
#erp_send_reauth_start=1

# EAP-Initiate/Re-auth-Start 的域名
...
- 集成的 EAP 服务器可用于本地鉴权而不需要外部的 RADIUS 服务
- 可用于本地 IEEE 802.1X/EAPOL 鉴权或外部 RADIUS 服务设备鉴权

```ini
# 使用集成 EAP 服务而不是外部 RADIUS 鉴权服务
eap_server=0

# EAP 服务用户数据库
# 如果启用了 sqlite 可设置为 "sqlite:/path/to/sqlite.db"
#eap_user_file=/etc/hostapd.eap_user

# EAP-TLS/PEAP/TTLS  CA 证书 PEM 或 DER 文件
#ca_cert=/etc/hostapd.ca.pem

# EAP-TLS/PEAP/TTLS 服务证书 PEM 或 DER 文件
#server_cert=/etc/hostapd.server.pem

# 服务证书的私钥 - 如果思考和证书在相同文件可能指向与 server_cert 相同
# PKCS#12 (PFX) (.p12/.pfx) 可不使用 server_cert 而只配置 private_key
#private_key=/etc/hostapd.server.prv

# 私钥密码
#private_key_passwd=secret passphrase

# 添加额外的服务端证书 - 和上面参数一样 - 多了后缀 2
# PEM 格式的 ca_cert 作为多个证书的根证书
# 多证书配置主要用于同时启用 RSA 和 ECC 公钥 - 选择基于客户端配置
# 如果加密算法选择错误可能会导致中断问题
#server_cert2=/etc/hostapd.server-ecc.pem
#private_key2=/etc/hostapd.server-ecc.prv
#private_key_passwd2=secret passphrase

# 服务端标示 - 作为 EAP 授权服务时提供 - 未设置使用 hostapd
...
# 配合 EAP-TTLS 和 EAP-FAST 使用
# 启用 Trusted Network Connect (TNC) - 在节点链接前进行 TNC 验证
#tnc=1

# 启用 EAP Re-authentication Protocol (ERP) - RFC 6696
#eap_server_erp=1
```

### RADIUS 客户端配置

- IEEE 802.1X 使用外部授权服务器

```ini
# AP 的 IP 地址 - 用于 NAS-IP-Address
own_ip_addr=127.0.0.1

# RADIUS 消息的 NAS-Identifier 字段 - NAS 内唯一
# hostapd 对每个 BSS 使用独立客户端 - 每个 BSS 都需要配置
# When using IEEE 802.11r, nas_identifier must be set and must be between 1 and 48 octets long.
# 务必为每个 BSS 设置唯一的值
...
#
# DAS Event-Timestamp 时间窗口 - 秒
#radius_das_time_window=300
#
# DAS 需要 Event-Timestamp
#radius_das_require_event_timestamp=1
#
# DAS 需要 Message-Authenticator
#radius_das_require_message_authenticator=1
```

### RADIUS 授权服务配置

- hostapd 可作为 RADIUS 授权服务
- 需要启动集成的 EAP 服务 - 两个共享配置

```ini
# RADIUS 服务的客户端配置 - 注释则不启用
...
- [WPA](https://en.wikipedia.org/wiki/Wi-Fi_Protected_Access) - Wi-Fi Protected Access

```ini
# 启用 WPA -  例如 WPA-PSK WPA-RADIUS/EAP
# WPA-PSK 需要 wpa_psk wpa_passphrase 且 wpa_key_mgmt 包含 WPA-PSK
# WPA-RADIUS/EAP 需要 ieee8021x 但不需要动态 WEP 密钥
#   必须配置 RADIUS 授权服务，wpa_key_mgmt 包含 WPA-EAP
# bit0 = WPA (IEEE 802.11i/D3.0)
# bit1 = WPA2 (full IEEE 802.11i/RSN) (dot11RSNAEnabled) / WPA3
# WPA3 对于 WPA3-Personal 需要  wpa_key_mgmt=SAE
#wpa=2

# WPA 密码 - 可以配置 PSK 位 256位的 16进制格式，也可以用明文密码 8-63 长度
# 明文会转换为 PSK - 会使用 SSID - 跟随变化
# wpa_psk (dot11RSNAConfigPSKValue)
# wpa_passphrase (dot11RSNAConfigPSKPassPhrase)
#wpa_psk=0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef
#wpa_passphrase=secret passphrase

# 可选 - 通过文件读取 WPA PSK - 格式为 PSM,MAC - 可配置多个
# 使用绝对地址，确保 SIGHUP 能再次读取
#wpa_psk_file=/etc/hostapd.wpa_psk

# 可选 - WPA 密码由 RADIUS 提供 - 需要 macaddr_acl 设置为 2
# 0 禁用 - 默认
# 1 可选 - 如果 RADIUS Tunnel-Password 没有则使用默认 密码/PSK
# 2 必须 - 如果 RADIUS Tunnel-Password 没有则拒绝
#wpa_psk_radius=0

# 接受的密钥管理算法 - 逗号分割 - WPA-PSK, WPA-EAP
...
# 密钥对(单播)接受的加密套件/加密算法 - 空格分割
# CCMP = AES in Counter mode with CBC-MAC (CCMP-128)
# TKIP = Temporal Key Integrity Protocol
# CCMP-256 = AES in Counter mode with CBC-MAC with 256-bit key
# GCMP = Galois/counter mode protocol (GCMP-128)
# GCMP-256 = Galois/counter mode protocol with 256-bit key
# 广播或多播的组加密算法基于配置进行自动选择 - 也可以配置 group_cipher
#   如果只允许 CCMP 那么组也会使用该算法，否则会使用 TKIP
# (dot11RSNAConfigPairwiseCiphersTable)
# WPA1 密钥加密算法 - 默认 TKIP
#wpa_pairwise=TKIP CCMP
# RSN/WPA2 密钥加密算法 - 默认使用 wpa_pairwise
#rsn_pairwise=CCMP

# 配置错误可能导致中断问题 - 不要配置 - 只用于测试
#group_cipher=CCMP
...
- BSS - https://en.wikipedia.org/wiki/Service_set_(802.11_network)
...
# 0 = Excluded - AP 不想 STA 使用蜂窝数据链接
# 1 = AP 倾向 STA 不使用使用蜂窝数据链接
# 255 = AP 倾向 STA 使用蜂窝数据链接
...
- Fast Session Transfer (FST) - IEEE Std 802.11ad-2012
  - 将会话从一个通道转换到另外一个通道
- 编译添加 CONFIG_FST
...
- 上述配置用于配置默认网卡 wlan# 或多 SSID VLAN
- 其它的 BSSID 可通过添加 bss 加默认网卡名字配置
- 会生成基于已配置内存生成 BSSID 的掩码 - 会验证 dev_addr & MASK == dev_addr
- 如果不匹配需要先配置 mac 地址 `ifconfig wlan0 hw ether <MAC addr>`
- 如果对于每个二级 BSS 都配置了 BSSID 则该限制不存在
